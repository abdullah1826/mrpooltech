...
const Productstable = () => {
    ...
    const updatedProductData = productData.map((item) => {
      if (item.id === id) {
        ...
        if (operation === "plus") {
          ...
          item.price =
            parseInt(item.price) + parseInt(localStorage.getItem(`${item.id}`));
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <img
          src={row.imgUrl ? row.imgUrl : "https://placehold.co/45"}
    ...
    rows.length > 0 && {
      ...
      selector: (row) => (
        <div className="h-[50px] w-[70px] flex items-center justify-between">
          <div
            className="h-[20px] w-[20px] flex justify-center items-center text-xl cursor-pointer bg-[#35A1CC] rounded-[4px] text-white"
            onClick={() => calculatingProducts(row.id, "minus")}
          >
            -
          </div>
          <div className="h-[20px] w-[20px] flex justify-center items-center text-lg ">
            {row.qty || 1}
          </div>
          <div
            className="h-[20px] w-[20px] flex justify-center items-center text-xl cursor-pointer bg-[#35A1CC] rounded-[4px] text-white"
            onClick={() => calculatingProducts(row.id, "plus")}
          >
            +
          </div>
        </div>
      ...
      cell: (row) => (
        <div className="flex ">
          <button
            ...
            onClick={() => showdescription(row.description)}
          >
            View
          </button>
        </div>
  ...
  return (
    <>
      ...
      <div className="flex w-[100%]">
        <Sidebar />
        <div className="relative h-[100vh] overflow-x-auto w-[100%]">
          {showmodal2 && (
            <Descriptionmodal
              description={mydescription}
              hidemodal={hisedescription}
            />
          )}
          ...
          <div className="absolute right-5 flex w-[100%] justify-end">
            {rows.length > 0 && (
              <Link to="/invoice">
                <div className="h-[45px] border w-[100%] mr-5  flex justify-center items-center bg-[#35A1CC] text-white cursor-pointer rounded-lg">
                  Create Invoice
                </div>
              </Link>
            )}
            <Link to="/addproducts">
              <div className="h-[45px] border w-[210px] ml-5 flex justify-center items-center bg-[#35A1CC] text-white cursor-pointer rounded-lg">
                Add new Product +
              </div>
            </Link>
          </div>
