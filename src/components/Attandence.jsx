  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex">
          <button
            className="h-[40px] w-[70px] border bg-[#35A1CC] rounded-md text-white "
            onClick={() => showLocation(row.id)}
          >
            View
          </button>
        </div>
...
//   console.log(siteList);
  ...
  //         user?.site?.toLowerCase().includes(search.toLowerCase())
  ...
  // useEffect(() => {
  //    let sDate =  formatDateToLong(filterValue)
  //     const result = mylist?.filter((user) => {
  //       return (
  //         user?.date == sDate
  ...
  function formatMonth(dateString) {
    ...
    const date = new Date(dateString + "-01"); // Ensure proper date format
    if (isNaN(date)) return "Invalid Date"; // Handle invalid dates
  ...
  useEffect(() => {
    const sDate = filterType === "date" ? formatDateToLong(filterValue) : "";
    const sMonth = filterType === "month" ? formatMonth(filterValue) : "";

    const result = mylist?.filter((user) => {
      const matchesSite =
        filterType === "site"
          ? filterValue
            ? user?.site === filterValue
            : true
          : true;
      const matchesDate = filterType === "date" ? user?.date === sDate : true;

      // Extract month and year from user.date
      const userMonth = formatMonth(user?.date); // Convert user's date to "Month Year" format
      const matchesMonth = filterType === "month" ? userMonth === sMonth : true;

      return matchesSite && matchesDate && matchesMonth;
    });

    setfiltered(result);
  }, [filterValue, search, mylist, filterType]);
  ...
  useEffect(() => {
    if (!filterType) return; // Avoid running when filterType is not set

    setFilterValue(""); // Reset filter value when switching filters
  ...
  const handleColumnSelection = (column) => {
    setSelectedColumns((prevSelectedColumns) => {
      const isSelected = prevSelectedColumns.some(
  ...
  const generatePDF = () => {
    ...
    const tableRows = selectedUsers.map((user) =>
      selectedColumns.map((col) => user[col.selector])
    );

    // Convert dates to a readable format
    const formattedRows = tableRows.map((row) =>
      row.map((cell) => (cell instanceof Date ? cell.toLocaleString() : cell))
    );
  ...
  const filteredColumns = columns.filter(
    (column) =>
      column.name !== "Checkin Location" && column.name !== "Checkout Location"
  );
  ...
  return (
    <>
      {showmod && (
        <Locationmodel
          checkinloc={loc.checkinLocation}
          checkoutloc={loc.checkoutLocation}
          hidemod={hidemodal}
        />
      )}
      <div className="flex w-[100%]">
        <Sidebar />
        <div className="relative overflow-x-auto w-[100%] h-[100vh]">
          <img src={upper} className="w-[100%]" alt="Header" />
            ...
            <div className="border w-[100%]">
              ...
              <div className=" w-[100%] flex items-start mt-5 justify-between p-2">
                ...
                <div className=" flex justify-around items-center z-10 left-[310px] top-[0px] w-[40%] p-2 bg-white shadow-md rounded-lg border">
                  {/* Dropdown Button */}
                  <div className="w-[auto]  relative" ref={dropdownRef}>
                    <button
                      className="border w-full px-3 py-1 rounded-md text-left bg-white flex justify-between items-center"
                    ...
                    {/* Dropdown Options */}
                    {showOptions && (
                      <div className="absolute left-0 w-full bg-white border mt-2 p-2 rounded-md shadow-md">
                        {["date", "month", "site"].map((option) => (
                          <button
                            key={option}
                  ...
                  {/* Filter Input Field */}
                  <div className="w-[33%] mt-0">
                    {filterType === "date" && (
                      <input
                        type="date"
                        className="border w-[100%] py-4 px-1 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    {filterType === "month" && (
                      <input
                        type="month"
                        className="border w-[100%] py-1 px-1 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        onChange={(e) =>
                          setFilterValue(formatMonth(e.target.value))
                        }
                        value={filterValue}
                      />
                    )}
                    ...
                    {filterType === "site" && (
                      ...
                      >
                        ...
                        {/* Mapping through sites dynamically */}
                        {mylist.length > 0 ? (
                          mylist.map((item, index) => (
                            <option
                              key={index}
                              value={item.site}
                              className="border w-[50px]  py-1 px-1 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                              {item.site}
                            </option>
                          ))
                  ...
                  <button
                    className="h-[35px] w-[20%] flex items-center justify-center rounded-xl bg-[#35A1CC] text-white"
                    onClick={() => fetchData()}
                  >
                    Clear
                  </button>
                ...
                <div className=" w-[20%]  top-[0px] flex justify-end z-20">
                  <button
                    className="h-[50px] w-[180px] flex items-center justify-center rounded-2xl bg-[#35A1CC] text-white ml-4"
      ...
      <Modal
        ...
        aria-labelledby="column-selection-modal"
      >
        <Box
          sx={{
            position: "absolute",
            ...
            <div className="w-[90%] flex justify-start  flex-col">
              ...
              <FormGroup>
                {columns
                  .filter(
                    (column) =>
                      !column.isImageColumn && !column.isLocationButtonColumn
                  ) // Filter out the image column
                  .map((column, index) => (
                    <FormControlLabel
                      key={index}
                      control={
      ...
      >
        <Box sx={imageModalStyle}>
          ...
          <img
            ...
            alt="Selected"
            className="max-h-[380px] max-w-[70%]"
          />
          <Button
            variant="contained"
            color="primary"
