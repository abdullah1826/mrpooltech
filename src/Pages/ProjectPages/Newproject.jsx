...
console.log(mylist)
    ...
    const columns = [
      ...
      { name: 'Owner', selector: (row) => { return row.owner }, sortable: true, width: '90px' },
    ...
    //   { name: 'Creation Date', selector: (row) => { return row.creationDate }, sortable: true, width: '130px' },
      // { name: 'Age', selector: 'age', sortable: true ,},
      {
        ...
        cell: (row) => (
            <div className='flex'>
                <button className='h-[40px] w-[70px] border bg-[#35A1CC] rounded-md text-white mr-2' onClick={() => viewUserData(row)}>View</button>
                <button className='h-[40px] w-[70px] border bg-[#35A1CC] rounded-md text-white mr-2' onClick={() => Editdata(row.id)}>Edit</button> 
                <button className='h-[40px] w-[70px] border bg-[#f44336] rounded-md text-white' onClick={() => { return modalseter(row.id) }}>Delete</button>
            </div>
        ),
        width: '250px',
    },
...
<div class="w-[100%] flex justify-center mb-5"><h2 class="text-4xl font-[500]  text-[#0b6e99]">New Projects</h2></div>
...
<div class="flex absolute z-10 right-6 mt-3"><Link to="/newinput"><div class="h-[45px] border w-[180px]  rounded-xl  flex justify-center items-center bg-[#35A1CC] text-white cursor-pointer">Add new site +</div></Link></div>
