  ...
  const addData = async () => {
    ...
    if (data.workerName && data.email && data.password) {
      try {
        ...
        // Save worker data to Firebase Database
        await update(ref(db, `workers/${user.uid}`), { ...data, id: user.uid });
        ...
        // If image is not a direct URL, upload it
        if (img && returnIfHttps(img) === false) {
          // console.log("Uploading profile image:", img);
    
          const storageRef = sRef(storage, `workers/${user.uid}.png`);
    
          if (img.startsWith("data:image")) {
            let base64Data = img.split(",")[1]; // Extract base64 content
            console.log("Uploading profile image:", img);
            ...
            // Update worker profile with the image URL
            await update(ref(db, `workers/${user.uid}`), { profileUrl: URL });
